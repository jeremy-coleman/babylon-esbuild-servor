...
export class Ocean {
  ...
  public async createScene(engine: BABYLON.Engine, canvas: HTMLCanvasElement): Promise<BABYLON.Scene> {
    ...
    ;(window as any).numbg = function (): void {
      console.log(
        ...
        BABYLON.WebGPUCacheBindGroups.NumBindGroupsCreatedTotal,
        " - NumBindGroupsCreatedLastFrame=",
        BABYLON.WebGPUCacheBindGroups.NumBindGroupsCreatedLastFrame
      )
    }
    ...
    this._gui = new OceanGUI(
      this._useProceduralSky,
      scene,
      engine,
      this._parameterRead.bind(this),
      this._parameterChanged.bind(this)
    )
  ...
  private _checkSupport(): boolean {
    ...
    const textNOk =
      "**Use WebGPU to watch this demo which requires compute shaders support. To enable WebGPU please use Chrome Canary or Edge canary. Also select the WebGPU engine from the top right drop down menu.**"
  ...
  private async _loadMeshes() {
    await BABYLON.SceneLoader.AppendAsync(
      "",
      "https://assets.babylonjs.com/meshes/babylonBuoy.glb",
      this._scene,
      undefined,
      ".glb"
    )
    ...
    this._buoyancy.addMesh(
      babylonBuoyRoot,
      {
        v1: new BABYLON.Vector3(0.7 / scale, 1 / scale, -1.5 / scale),
        v2: new BABYLON.Vector3(0.7 / scale, 1 / scale, 1.5 / scale),
        v3: new BABYLON.Vector3(-1.5 / scale, 1 / scale, -1.5 / scale),
      },
      0.0,
      2
    )
    ...
    this._lightBuoy.diffuse = new BABYLON.Color3(0.96, 0.7, 0.15).toLinearSpace()
  ...
  private _parameterChanged(name: string, value: any): void {
    ...
    if (name.startsWith("procSky_")) {
      name = name.substring(8)
      this._setValue(
        this._skybox.skyMaterial,
        name,
        value === false ? false : value === true ? true : parseFloat(value)
      )
...
class OceanGUI {
  ...
  constructor(
    hasProceduralSky: boolean,
    scene: BABYLON.Scene,
    engine: BABYLON.Engine,
    paramRead: (name: string) => any,
    paramChanged: (name: string, value: any) => void
  ) {
    this._scene = scene
    this._visible = true
...
class OceanGeometry {
  ...
  private _updatePositions(): void {
    ...
    for (let i = 0; i < this.clipLevels; i++) {
      ...
      trimPosition
        .copyFrom(snappedPosition)
        .addInPlace(centerOffset)
    ...
    if (this.useSkirt) {
      ...
      this._skirt.position
        .copyFrom(previousSnappedPosition)
  ...
  private _offsetFromCenter(level: number, activeLevels: number, result: BABYLON.Vector3): void {
    ...
    const v =
      (((((1 << this.clipLevels) +
        OceanGeometry._GeometricProgressionSum(2, 2, this.clipLevels - activeLevels + level + 1, this.clipLevels - 1)) *
        this.lengthScale) /
        k) *
        (k - 1)) /
      2
  ...
  private _instantiateMeshes(): void {
    ...
    this._center = this._instantiateElement(
      "Center",
      this._createPlaneMesh(2 * k, 2 * k, 1, Seams.All),
      this._materials[this._materials.length - 1]
    )
    ...
    if (this.useSkirt) {
      this._skirt = this._instantiateElement(
        "Skirt",
        this._createSkirtMesh(k, this.skirtSize),
        this._materials[this._materials.length - 1]
      )
    }
  }
  ...
  private _createPlaneMesh(
    width: number,
    height: number,
    lengthScale: number,
    seams: Seams = Seams.None,
    trianglesShift = 0
  ): BABYLON.Mesh {
  ...
  public async getMaterial(useMid: boolean, useClose: boolean, useNodeMaterial = false): Promise<BABYLON.Material> {
    ...
    if (!useNodeMaterial) {
      ...
      mat.AddUniform(
        "_CameraData",
        "vec4",
        new BABYLON.Vector4(this._camera.minZ, this._camera.maxZ, this._camera.maxZ - this._camera.minZ, 0)
      )
      ...
      ;(mat.getBlockByName("Displacement_c0") as BABYLON.TextureBlock).texture = this._wavesGenerator.getCascade(0)
        .displacement as BABYLON.Texture
      ;(mat.getBlockByName("Derivatives_c0") as BABYLON.TextureBlock).texture = this._wavesGenerator.getCascade(0)
        .derivatives as BABYLON.Texture
...
class SkyBox {
  ...
  constructor(useProcedural: boolean, scene: BABYLON.Scene) {
    ...
    this._skybox = BABYLON.MeshBuilder.CreateBox(
      "skyBox",
      { size: 1000.0, sideOrientation: BABYLON.Mesh.BACKSIDE },
      this._scene
    )
  ...
  private _initSkybox(): void {
    ...
    const reflectionTexture = new BABYLON.HDRCubeTexture(
      "https://popov72.github.io/BabylonDev/resources/webgpu/oceanDemo/0c03bd6e3c9d04da0cf428bbf487bf68.hdr",
      this._scene,
      256,
      false,
      true,
...
class Buoyancy {
  ...
  public addMesh(mesh: BABYLON.TransformNode, frame: BuoyancyFrame, yOffset = 0, spaceCoordinates = 0): void {
    this._meshes.push({
      ...
      yOffset,
      spaceCoordinates,
      initQuaternion: mesh.rotationQuaternion!.clone(),
      curStep: 0,
      curQuaternion: new BABYLON.Quaternion(),
      stepQuaternion: new BABYLON.Quaternion(),
    })
  }
  ...
  private _updateMesh(meshBuoyancy: MeshBuoyancy): void {
    ...
    const { mesh, frame, yOffset, spaceCoordinates, initQuaternion, curQuaternion, stepQuaternion, curStep } =
      meshBuoyancy
...
class WavesGenerator {
  ...
  private _generateNoiseTexture(size: number, noiseBuffer: BABYLON.Nullable<ArrayBuffer>): BABYLON.Texture {
    ...
    const noise = new BABYLON.RawTexture(
      ...
      size,
      size,
      numChannels === 2 ? BABYLON.Constants.TEXTUREFORMAT_RG : BABYLON.Constants.TEXTUREFORMAT_RGBA,
      this._engine,
      false,
      false,
...
class WavesCascade {
  ...
  constructor(
    size: number,
    gaussianNoise: BABYLON.BaseTexture,
    fft: FFT,
    rttDebug: RTTDebug,
    debugFirstIndex: number,
    engine: BABYLON.Engine
  ) {
    this._engine = engine
    this._size = size
    ...
    this._timeDependentSpectrum = new BABYLON.ComputeShader(
      "timeDependentSpectrumCS",
      this._engine,
      { computeSource: timeDependentSpectrumCS },
      {
        bindingsMapping: {
    ...
    this._buffer = ComputeHelper.CreateStorageTexture(
      "buffer",
      this._engine,
      this._size,
      this._size,
    ...
    this._DxDz = ComputeHelper.CreateStorageTexture(
      "DxDz",
      this._engine,
      this._size,
      this._size,
    ...
    this._DyDxz = ComputeHelper.CreateStorageTexture(
      "DyDxz",
      this._engine,
      this._size,
      this._size,
    ...
    this._DyxDyz = ComputeHelper.CreateStorageTexture(
      "DyxDyz",
      this._engine,
      this._size,
      this._size,
    ...
    this._DxxDzz = ComputeHelper.CreateStorageTexture(
      "DxxDzz",
      this._engine,
      this._size,
      this._size,
    ...
    this._texturesMerger = new BABYLON.ComputeShader(
      "texturesMerger",
      this._engine,
      { computeSource: wavesTexturesMergerCS },
      {
        bindingsMapping: {
    ...
    this._displacement = ComputeHelper.CreateStorageTexture(
      "displacement",
      this._engine,
      this._size,
      this._size,
    ...
    this._derivatives = ComputeHelper.CreateStorageTexture(
      "derivatives",
      this._engine,
      this._size,
      this._size,
    ...
    this._turbulence = ComputeHelper.CreateStorageTexture(
      "turbulence",
      this._engine,
      this._size,
      this._size,
    ...
    this._turbulence2 = ComputeHelper.CreateStorageTexture(
      "turbulence",
      this._engine,
      this._size,
      this._size,
  ...
  public calculateWavesAtTime(time: number): void {
    ...
    this._texturesMerger.setTexture(
      "TurbulenceRead",
      this._pingPongTurbulence ? this._turbulence : this._turbulence2,
      false
    )
    this._texturesMerger.setStorageTexture(
      "TurbulenceWrite",
      this._pingPongTurbulence ? this._turbulence2 : this._turbulence
    )
    ...
    this._engine.generateMipmaps(
      this._pingPongTurbulence ? this._turbulence2.getInternalTexture()! : this._turbulence.getInternalTexture()!
    )
  }
...
class InitialSpectrum {
  ...
  constructor(
    ...
    this._phase1 = new BABYLON.ComputeShader(
      "initialSpectrum",
      this._engine,
      { computeSource: initialSpectrumCS },
      {
        bindingsMapping: {
    ...
    this._initialSpectrum = ComputeHelper.CreateStorageTexture(
      "h0",
      engine,
      textureSize,
      textureSize,
    ...
    this._spectrumParameters = new BABYLON.StorageBuffer(
      this._engine,
      8 * 2 * 4,
    ...
    this._phase2 = new BABYLON.ComputeShader(
      "initialSpectrum2",
      this._engine,
      { computeSource: initialSpectrum2CS },
      {
        bindingsMapping: {
...
class FFT {
  ...
  constructor(engine: BABYLON.Engine, scene: BABYLON.Scene, rttDebug: RTTDebug, debugFirstIndex: number, size: number) {
    ...
    const cs = new BABYLON.ComputeShader(
      "computeTwiddleFactors",
      this._engine,
      { computeSource: fftPrecomputeCS },
      {
        bindingsMapping: {
  ...
  private _createComputeShaders(): void {
    for (let i = 0; i < 2; ++i) {
      this._horizontalStepIFFT[i] = new BABYLON.ComputeShader(
        "horizontalStepIFFT",
        this._engine,
        { computeSource: fftInverseFFTCS },
        {
          bindingsMapping: {
            params: { group: 0, binding: 1 },
      ...
      this._verticalStepIFFT[i] = new BABYLON.ComputeShader(
        "verticalStepIFFT",
        this._engine,
        { computeSource: fftInverseFFT2CS },
        {
          bindingsMapping: {
            params: { group: 0, binding: 1 },
            PrecomputedData: { group: 0, binding: 3 },
    ...
    this._permute = new BABYLON.ComputeShader(
      "permute",
      this._engine,
      { computeSource: fftInverseFFT3CS },
      {
        bindingsMapping: {
          InputBuffer: { group: 0, binding: 5 },
          OutputBuffer: { group: 0, binding: 6 },
...
class ComputeHelper {
  ...
  static GetThreadGroupSizes(source: string, entryPoint: string): BABYLON.Vector3 {
    ...
    return res
      ? new BABYLON.Vector3(parseInt(res[1]), parseInt(res[2]), parseInt(res[3]))
      : new BABYLON.Vector3(1, 1, 1)
  }

  static CreateStorageTexture(
    name: string,
    sceneOrEngine: BABYLON.Scene | BABYLON.ThinEngine,
    nwidth: number,
    nheight: number,
    textureFormat = BABYLON.Constants.TEXTUREFORMAT_RGBA,
    textureType = BABYLON.Constants.TEXTURETYPE_FLOAT,
    filterMode = BABYLON.Constants.TEXTURE_NEAREST_SAMPLINGMODE,
    generateMipMaps = false,
    wrapMode = BABYLON.Constants.TEXTURE_WRAP_ADDRESSMODE,
    texture: BABYLON.Nullable<BABYLON.Texture> = null
  ): BABYLON.Texture {
  ...
  static CopyTexture(source: BABYLON.BaseTexture, dest: BABYLON.BaseTexture, engine_?: BABYLON.Engine): void {
    ...
    if (
      (!ComputeHelper._copyTexture4CS && numChannels === 4) ||
      (!ComputeHelper._copyTexture2CS && numChannels === 2)
    ) {
      ...
      const cs1 = new BABYLON.ComputeShader(
        ...
        {
          computeSource:
            numChannels === 4 ? ComputeHelper._copyTexture4ComputeShader : ComputeHelper._copyTexture2ComputeShader,
        },
        {
          bindingsMapping: {
  ...
  static CopyBufferToTexture(source: BABYLON.StorageBuffer, dest: BABYLON.BaseTexture): void {
    if (!ComputeHelper._copyBufferTextureCS) {
      ...
      const cs1 = new BABYLON.ComputeShader(
        ...
        engine,
        { computeSource: ComputeHelper._copyBufferTextureComputeShader },
        {
          bindingsMapping: {
            dest: { group: 0, binding: 0 },
            src: { group: 0, binding: 1 },
  ...
  static CopyTextureToBuffer(source: BABYLON.BaseTexture, dest: BABYLON.StorageBuffer): void {
    if (!ComputeHelper._copyTextureBufferCS) {
      ...
      const cs1 = new BABYLON.ComputeShader(
        ...
        engine,
        { computeSource: ComputeHelper._copyTextureBufferComputeShader },
        {
          bindingsMapping: {
  ...
  static ClearTexture(source: BABYLON.BaseTexture, color: BABYLON.Color4): void {
    if (!ComputeHelper._clearTextureCS) {
      ...
      const cs1 = new BABYLON.ComputeShader(
        "clearTextureCompute",
        engine,
        { computeSource: ComputeHelper._clearTextureComputeShader },
        {
          bindingsMapping: {
  ...
  static Dispatch(cs: BABYLON.ComputeShader, numIterationsX: number, numIterationsY = 1, numIterationsZ = 1): void {
    if (!(cs as any).threadGroupSizes) {
      ;(cs as any).threadGroupSizes = ComputeHelper.GetThreadGroupSizes(
        cs.shaderPath.computeSource,
        cs.options.entryPoint ?? "main"
      )
    }
...
class Vector3Float32 extends BABYLON.Vector3 {
  ...
  public addToRef(otherVector: BABYLON.DeepImmutable<Vector3Float32>, result: Vector3Float32): Vector3Float32 {
    return result.copyFromFloats(
      fp32(this._x + otherVector._x),
      fp32(this._y + otherVector._y),
      fp32(this._z + otherVector._z)
    )
  }
   ...
   */
  public static Lerp(
    start: BABYLON.DeepImmutable<Vector3Float32>,
    end: BABYLON.DeepImmutable<Vector3Float32>,
    amount: number
  ): Vector3Float32 {
      ...
      button.onPointerUpObservable.add(() => {
        ...
        if (texture) {
          ...
          texture.readPixels()!.then((buffer) => {
            const channels =
              textureFormat === BABYLON.Constants.TEXTUREFORMAT_R
                ? ["R"]
                : textureFormat === BABYLON.Constants.TEXTUREFORMAT_RG
                ? ["R", "G"]
                : ["R", "G", "B", "A"]
            this._exrSerializer.serialize(
...
const texColorCst = new Vector3Float32(
  f32(f32(0.0) * f32(0.3)),
  f32(f32(0.001) * f32(0.3)),
  f32(f32(0.0025) * f32(0.3))
)

;(SkyMaterial.prototype as any).getSunColor = function () {
  const sunIntensity = (zenithAngleCos: number) => {
    return f32(
      EE * Math.max(0.0, f32(1.0 - f32(Math.exp(-f32(cutoffAngle - f32(Math.acos(zenithAngleCos))) / f32(steepness)))))
    )
  }
  ...
  const hgPhase = (cosTheta: number, g: number) => {
    return f32(
      f32(1.0 / f32(4.0 * PI)) *
        f32(
          f32(1.0 - f32(Math.pow(g, 2.0))) /
            f32(Math.pow(1.0 - f32(f32(2.0 * g) * cosTheta) + f32(Math.pow(g, 2.0)), 1.5))
        )
    )
  }
  ...
  const sR = f32(
    rayleighZenithLength /
      f32(
        f32(Math.cos(zenithAngle)) +
